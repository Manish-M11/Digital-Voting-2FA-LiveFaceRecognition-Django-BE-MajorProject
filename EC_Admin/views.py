    ...
    if request.method == 'POST':
        ...
        if (Election.objects.filter(election_id=election_id).exists()):
            ...
            if election_type == 'PC-GENERAL':
                c = Candidates.objects.filter(
                    state=state, constituency='Parliamentary')
                for i in c:
                ...
                c = Candidates.objects.filter(
                    state=state, constituency='Assembly')
                for i in c:
                    candidates_votes = Votes(election_id=election_id, candidate_id=i.candidate_id,
                                             candidate_name=i.name, candidate_party=i.candidate_party, state=i.state,
                                             constituency=i.parliamentary)
...
def submitevmvote(request):
    ...
    for i in candidate_id:
        v = Votes.objects.get(
            election_id=viewcandidatesforaddvote.election_id, candidate_id=i)
...
def generate_report(request):
    ...
    if Voted.objects.filter(election_id=election_id, state=state, constituency=constituency):
        vvotedall = Voted.objects.filter(
            election_id=election_id, state=state, constituency=constituency)
        e_male = 0
        e_female = 0
        e_others = 0
        e_total = 0
        ...
        if Voted.objects.filter(election_id=election_id, state=state, constituency=constituency, has_voted=has_voted):
            vvotedyes = Voted.objects.filter(
                election_id=election_id, state=state, constituency=constituency, has_voted=has_voted)
            v_male = 0
            v_female = 0
            v_others = 0
            v_total = 0
            ...
            if e_total != 0:
                ...
                p_total = 0
                poll_total = float("{:.2f}".format(p_total))
            report = Reports(election_id=election_id, state=state, constituency=constituency, electors_male=e_male, electors_female=e_female, electors_others=e_others, electors_total=e_total,
                             voters_male=v_male, voters_female=v_female, voters_others=v_others, voters_total=v_total, poll_male=poll_male, poll_female=poll_female, poll_others=poll_others, poll_total=poll_total)
            report.save()
...
def view_voter(request):
    if request.method == 'POST':
        ...
        constituency2 = request.POST['constituency2']
        if constituency1 == 'Parliamentary':
            voters = Voters.objects.filter(
                state=state, parliamentary=constituency2)
